...
class ConnScan(Extract):
    ...
    def __init__(self, file: File) -> None:
        ...
        header.insert(self.Columns.local_port, "Local Port")
        header.insert(self.Columns.remote_port, "Remote Port")

        data = [row.split(',') for row in lines[2:]]
        for index, row in enumerate(data):
            laddr, lport = data[index][self.Columns.local_address].split(':')
            data[index].insert(self.Columns.local_port, lport)
            data[index][self.Columns.local_address] = laddr

            raddr, rport = data[index][self.Columns.remote_address].split(':')
            data[index].insert(self.Columns.remote_port, rport)
            data[index][self.Columns.remote_address] = raddr
    ...
    def visualize(self):
        ...
        laddr_groups = self.df.groupby('Local Address').groups
        g.attr(compound='true')

        for index, (laddr, group) in enumerate(laddr_groups.items()):
            with g.subgraph(name=f'host#{laddr}') as c:
                c.attr(rank='same', style='filled', fillcolor='red')
                c.node(f'node#{laddr}', f"{laddr}", shape='plaintext', fontcolor=Theme.tc)
